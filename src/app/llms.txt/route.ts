...
const LLMS_TXT = `# Canvascope
> canvascope.org

Canvascope is a local-first academic productivity product for students. The
flagship product is the free Canvascope Chrome extension for searching Canvas
and Brightspace coursework. Supporting products include the Canvascope web
workspace, Lectra Notes for iPhone and iPad, and Canvascope Research.

## Current Extension

Current extension version: v10.1.0.

Canvascope 10.1 is a Chrome MV3 extension for Canvas and Brightspace. It
...
Canvascope is local-first by default. The core LMS search index lives in browser
storage. Connected flows are explicit and user-directed.

Canvascope does not sell user data and does not include an advertising business
...
Canvascope includes a unified side-panel assistant and Course Brain workflows.
...
Canvascope also includes a multi-step agent. Instead of a single answer, it runs
...
first. If unavailable, Canvascope can use an authenticated Supabase Gemini
...
Lectra Notes is Canvascope's App Store app for document import, course PDFs,
Apple Pencil annotation, private on-device intelligence, backup, and
finished-file handoff back to Canvascope. It is available as a free download for
...
- Canvascope document metadata and sync state
- iOS Share Extension receive flow
- account deletion support and App Store privacy manifests

## DropBridge v3

The current Lectra -> Canvascope receive path uses DropBridge v3 concepts in the
extension runtime:
...
Canvascope -> Lectra document sends use the shared Lectra document storage and
synced-items contract. Canvascope 10.1 also includes an Attach from Lectra
...
The Canvascope web app provides public product pages, auth/session endpoints,
...
(terminal, git, Python) gaining a coding agent and SSH, the Canvascope AI agent
(multi-step tool use with a daily briefing and integrity-gated actions), the
Lectra Notes App Store launch, Canvascope 10.1
two-way Lectra workflows, Lectra on-device intelligence and Shortcuts hooks,
Chrome Gemini Nano on-device AI in Canvascope, Student Profiles, RISC account
...
Canvascope supports Google Cross-Account Protection through a website proxy route
...
Canvascope's current public position is:
...
- two-way document workflow between Canvascope browser surfaces and Lectra on Apple devices
...
Canvascope Research is the company's computational neuroscience program, published
